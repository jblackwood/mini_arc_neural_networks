...
class TaskTokenModel(nn.Module):
    """Task token model that converts task indices to one-hot vectors using Gumbel softmax.
    
    Takes task indices, looks up embeddings, passes through MLP layers,
    and converts to one-hot vectors using Gumbel softmax.
    """
    
    def __init__(self, num_tasks: int, num_task_latent_tokens: int, num_token_categories: int, d_model: int):
        """Initialize task token model.
        
        Args:
            num_tasks: Number of unique tasks
            num_task_latent_tokens: Number of latent tokens per task
            num_token_categories: Number of discrete categories for quantization
            d_model: Model dimension for intermediate processing
        """
        super().__init__()
    ...
    def forward(self, task_indices: torch.Tensor, temperature: float, hard: bool) -> torch.Tensor:
        ...
        Returns:
            Tensor of shape (batch_size, num_task_latent_tokens, num_token_categories)
        """
        batch_size = task_indices.shape[0]
        
        # Look up task embeddings: (batch_size, d_model)
        task_emb = self.task_embedding(task_indices)
        
        # Pass through MLP: (batch_size, num_task_latent_tokens * num_token_categories)
        logits = self.mlp(task_emb)
        
        # Reshape to (batch_size, num_task_latent_tokens, num_token_categories)
        logits = logits.view(batch_size, self.num_task_latent_tokens, self.num_token_categories)
        
        # Apply Gumbel softmax
        if self.training:
            one_hot = nn.functional.gumbel_softmax(logits, tau=temperature, hard=hard, dim=-1)
        else:
            # During eval, use argmax (hard one-hot)
            indices = logits.argmax(dim=-1)
            one_hot = nn.functional.one_hot(indices, num_classes=self.num_token_categories).float()
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            num_token_categories: Number of discrete categories for task tokens
            num_task_latent_tokens: Number of task latent tokens
        """
        super().__init__()

        # Store vocab_size and num_task_latent_tokens for later use
        self.vocab_size = vocab_size
        self.num_task_latent_tokens = num_task_latent_tokens
        ...
        # Task token prediction: (batch_size, num_task_latent_tokens, d_model) -> (batch_size, num_task_latent_tokens, num_token_categories)
        self.task_output_proj = nn.Linear(d_model, num_token_categories)
        self.num_token_categories = num_token_categories

    def forward(self, x: torch.Tensor, task_one_hot: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass.

        Args:
            x: Input tensor of shape (batch_size, 50) - tokens for input and output grids
            task_one_hot: Task one-hot vectors of shape (batch_size, num_task_latent_tokens, num_token_categories)

        Returns:
            Tuple of:
            - grid_logits: Tensor of shape (batch_size, 50, vocab_size) with logits for grid token prediction
            - task_logits: Tensor of shape (batch_size, num_task_latent_tokens, num_token_categories) with logits for task token prediction
        """
        ...
        # Predict task tokens: (batch_size, num_task_latent_tokens, d_model) -> (batch_size, num_task_latent_tokens, num_token_categories)
...
def optimize_output_grid(
    ...
    task_token_model: TaskTokenModel,
    ...
    Args:
        ...
        task_token_model: The task token model to generate task embeddings
...
def evaluate_denoising_accuracy(
    ...
    opt_result = optimize_output_grid(
        model=model,
        task_token_model=task_token_model,
        x_input=x_masked,
...
def create_masked_inputs(
    ...
    Randomly masks 1-25 positions across task one-hot tokens, input grid, and output grid.
    
    Args:
        task_one_hot: Task one-hot tensor of shape (batch_size, num_task_latent_tokens, num_token_categories)
    ...
    Returns:
        Tuple of (masked_task_one_hot, masked_grid_tokens, mask_positions)
        - masked_task_one_hot: (batch_size, num_task_latent_tokens, num_token_categories)
    ...
    # Clone inputs
    masked_task_one_hot = task_one_hot.clone()
    ...
    # Apply task masks: set masked positions to zero
    masked_task_one_hot = masked_task_one_hot * (~task_mask).unsqueeze(-1).float()
    ...
    return masked_task_one_hot, masked_grid_tokens, mask_positions
...
def compute_loss_for_batch(
    ...
    Args:
        ...
        task_token_model: The task token model
    ...
    grid_logits, task_logits = model(masked_grid_tokens, masked_task_one_hot)
    # grid_logits: (batch_size, 50, vocab_size)
    # task_logits: (batch_size, num_task_latent_tokens, num_token_categories)
    ...
    if task_mask.any():
        # Get targets from original task_one_hot
        task_targets = torch.argmax(task_one_hot, dim=-1)  # (batch_size, num_task_latent_tokens)
        
        # Flatten and select only masked positions
        task_logits_flat = task_logits.reshape(-1, num_token_categories)  # (batch_size * num_task_latent_tokens, num_token_categories)
...
def train_epoch(
    ...
    for examples in train_loader:
        ...
        torch.nn.utils.clip_grad_norm_(
            list(model.parameters()) + list(task_token_model.parameters()), 
            max_norm=1.0
        )
        optimizer.step()
...
def train(config: Config):
    ...
    task_token_model = cast(TaskTokenModel, torch.compile(task_token_model))
    model = cast(TransformerModel, torch.compile(model))

    # Count parameters
    task_token_params = sum(p.numel() for p in task_token_model.parameters() if p.requires_grad)
    transformer_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_params = task_token_params + transformer_params
    
    task_emb_params = sum(p.numel() for p in task_token_model.task_embedding.parameters() if p.requires_grad)
    rope_params = sum(p.numel() for p in model.rope.parameters() if p.requires_grad)
    
    embedding_params = task_emb_params + rope_params
    other_params = total_params - embedding_params
    
    print(f"Total trainable parameters: {total_params:,}")
    print(f"  TaskTokenModel: {task_token_params:,} ({task_token_params/total_params*100:.1f}%)")
    print(f"  TransformerModel: {transformer_params:,} ({transformer_params/total_params*100:.1f}%)")
    ...
    task_embedding_params = list(task_token_model.task_embedding.parameters())
    other_params = [
        *[p for n, p in task_token_model.named_parameters() if 'task_embedding' not in n],
    ...
    if config.load_model_path:
        if Path(config.load_model_path).exists():
            ...
            task_token_model.load_state_dict(checkpoint["task_token_model_state_dict"])
    ...
    for epoch in range(start_epoch, start_epoch + config.num_epochs):
        ...
        # Embedding layers
        task_emb_mean_sq = task_token_model.task_embedding.weight.pow(2).mean().item()
        ...
        writer.add_scalar("Temperature/gumbel", temperature, epoch)
        ...
        if (epoch + 1) % config.checkpoint_save_interval == 0:
            ...
            torch.save(
                {
                    "epoch": epoch + 1,
                    "task_token_model_state_dict": task_token_model.state_dict(),
