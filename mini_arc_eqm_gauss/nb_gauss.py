...
def optimize_output_grid(
    ...
    updates them using the model's predicted noise gradient. Only updates the
    last 25 tokens (output grid), not the task embedding or input grid.

    Args:
        model: The transformer model to use for computing noise gradients
    ...
    with torch.no_grad():
        ...
        for iteration in range(num_iterations):
            ...
            improved_mask_expanded = improved_mask.view(batch_size, 1, 1).expand_as(output_grid)
            best_output = torch.where(improved_mask_expanded, output_grid, best_output)
...
def compute_loss_for_batch(
    ...
    x = torch.cat([task_emb, token_emb], dim=1)  # (batch_size, 51, d_model)

    # Sample random gamma uniformly from [0, 1]
...
def main():
    ...
    config = Config(
        ...
        output_dir=Path("output/mini_arc_eqm_gauss"),
        test_ratio=0.2,
        random_seed=42,
