...
class TransformerModel(nn.Module):
    """Autoregressive GPT-style transformer for ARC tasks."""
    ...
    def forward(self, x: torch.Tensor, task_idx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass with autoregressive causal masking.
        ...
        Returns:
            ...
            - task_logits: Tensor of shape (batch_size, num_task_latent_tokens, task_codebook_size) with logits for task token prediction
        """
        batch_size = x.shape[0]
        
        # Get task embeddings and reshape
        task_emb = self.task_embedding(task_idx)  # (batch_size, task_codebook_dim * num_task_latent_tokens)
        task_embeddings = task_emb.view(batch_size, self.num_task_latent_tokens, self.task_codebook_dim)
        ...
        seq_len = self.num_task_latent_tokens + 50
        causal_mask = torch.triu(torch.ones(seq_len, seq_len, device=x.device), diagonal=1).bool()
        # TransformerEncoder uses mask where True means "cannot attend"
        
        # Apply transformer encoder with causal mask
        x = self.transformer_encoder(x, mask=causal_mask)  # (batch_size, num_task_latent_tokens+50, d_model)
...
def optimize_output_grid(
    ...
    """Generate output grid autoregressively by sampling tokens left-to-right.
    ...
    with torch.no_grad():
        ...
        # Initialize output grid with mask tokens (token value 10)
        x_current[:, 25:] = 10  # Set all output grid positions to mask token
        
        # Generate each output token autoregressively (positions 25-49)
        for position in range(25, 50):
            # Forward pass through model
            grid_logits, task_logits = model(x_current, task_indices)
            # grid_logits: (batch_size, 50, vocab_size)
            
            # Get logits for the current position
            position_logits = grid_logits[:, position, :]  # (batch_size, vocab_size)
            
            # Sample from the distribution (argmax for greedy sampling)
            predicted_tokens = torch.argmax(position_logits, dim=1)  # (batch_size,)
            
            # Update the current position with the predicted token
            x_current[:, position] = predicted_tokens
...
def compute_loss_for_batch(
    ...
    """Compute loss for a single batch using autoregressive next-token prediction.
    ...
    grid_logits, task_logits = model(x, task_indices)
    # grid_logits: (batch_size, 50, vocab_size)
    # task_logits: (batch_size, num_task_latent_tokens, task_codebook_size)

    # Compute task token targets by finding closest codebook entry for all task tokens
    task_codebook = cast(torch.Tensor, model.task_codebook)
    
    # Reshape task embeddings (already normalized above)
    ...
    similarities = torch.einsum('bd,cd->bc', task_emb_flat, task_codebook)  # (batch * tokens, codebook_size)
    distances = 2.0 - 2.0 * similarities
    
    # Get targets
    task_targets = torch.argmin(distances, dim=-1)  # (batch * tokens,)
    task_targets = task_targets.view(-1, num_task_latent_tokens)  # (batch_size, num_task_latent_tokens)
    
    # Compute task token loss (on all task tokens)
    task_logits_flat = task_logits.reshape(-1, task_codebook_size)  # (batch * tokens, task_codebook_size)
    task_targets_flat = task_targets.reshape(-1)  # (batch * tokens,)
    ...
    # Compute grid token loss (on all grid tokens)
    ...
    # Total loss averaged over all tokens
    total_tokens = task_logits_flat.shape[0] + grid_logits_flat.shape[0]
    loss = (task_loss + grid_loss) / total_tokens
