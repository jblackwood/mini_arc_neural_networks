...
class TransformerModel(nn.Module):
    """Denoising VAE transformer for ARC tasks."""
    ...
    def __init__(
        ...
        # Make token embeddings orthogonal using QR decomposition
        with torch.no_grad():
            # Get the embedding matrix: (vocab_size, d_model)
            embedding_matrix = self.token_embedding.weight.data
            
            # Compute QR decomposition
            # Q will be (vocab_size, vocab_size) orthogonal, R will be (vocab_size, d_model)
            # We want orthonormal rows, so we transpose, do QR, then transpose back
            q, r = torch.linalg.qr(embedding_matrix.T)  # q: (d_model, vocab_size), r: (vocab_size, vocab_size)
            
            # The orthonormal vectors are the columns of q, so we transpose to get rows
            orthogonal_embeddings = q.T[:vocab_size, :]  # (vocab_size, d_model)
            
            # Scale to look like N(0,1): multiply by sqrt(d_model)
            # Orthonormal vectors have norm 1, but N(0,1) vectors have expected norm sqrt(d_model)
            orthogonal_embeddings = orthogonal_embeddings * torch.sqrt(torch.tensor(d_model, dtype=torch.float32))
            
            # Set the orthogonalized embeddings
            self.token_embedding.weight.data = orthogonal_embeddings
        ...
        # Encoder to latent projections (mean and log variance)
        # Project from (51 * d_model) to latent_dim
        self.encoder_to_mu = nn.Linear(51 * d_model, latent_dim)
        self.encoder_to_logvar = nn.Linear(51 * d_model, latent_dim)

        # Latent to decoder projection
        self.latent_to_decoder = nn.Linear(latent_dim, 51 * d_model)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple of:
                - Tensor of shape (batch_size, 51, d_model) - denoised reconstruction
        ...
        output_grid = x[:, 26:, :]  # (batch_size, 25, d_model)
        ...
        # Concatenate
        encoder_input = torch.cat([task_token, input_grid, output_grid], dim=1)  # (batch_size, 51, d_model)

        # Apply encoder transformer
        encoded = self.encoder_transformer(encoder_input)  # (batch_size, 51, d_model)

        # Flatten for latent projection
        encoded_flat = encoded.view(batch_size, -1)  # (batch_size, 51 * d_model)

        # Project to latent space (mean and log variance)
        mu = self.encoder_to_mu(encoded_flat)  # (batch_size, latent_dim)
        logvar = self.encoder_to_logvar(encoded_flat)  # (batch_size, latent_dim)

        # Reparameterization trick
        z = self.reparameterize(mu, logvar)  # (batch_size, latent_dim)

        # Project back to decoder input
        decoder_input = self.latent_to_decoder(z)  # (batch_size, 51 * d_model)
        decoder_input = decoder_input.view(batch_size, 51, self.d_model)  # (batch_size, 51, d_model)

        # Apply decoder transformer
        decoded = self.decoder_transformer(decoder_input)  # (batch_size, 51, d_model)

        # Apply output projection
        output = self.output_projection(decoded)  # (batch_size, 51, d_model)

        # Compute KL divergence loss: -0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
...
def optimize_output_grid(
    ...
    """Optimize the output grid by iteratively denoising with the VAE.

    Initializes output grid embeddings as random gaussian noise and iteratively
    replaces the last 25 tokens with the model's output. Only updates the
    last 25 tokens (output grid), not the task embedding or input grid.
    ...
    with torch.no_grad():
        ...
        # Lookup input grid token embeddings (stays fixed during optimization)
        ...
        output_grid = torch.randn(batch_size, 25, d_model, device=device)
        ...
        for iteration in range(num_iterations):
            # Concatenate: task token, input grid, output grid
            x = torch.cat([task_emb, input_grid, output_grid], dim=1)  # (batch_size, 51, d_model)

            # Get model's denoised output (ignore kl_loss during inference)
            denoised, _ = model(x)  # (batch_size, 51, d_model)

            # Extract the denoised output grid (last 25 tokens)
            new_output_grid = denoised[:, -25:, :]  # (batch_size, 25, d_model)

            # Calculate MSE change from previous output grid
            change_per_sample = (new_output_grid - output_grid).pow(2).mean(dim=(1, 2))  # (batch_size,)
            ...
            improved_mask_expanded = improved_mask.view(batch_size, 1, 1).expand_as(new_output_grid)
        ...
        # Decode the best output embeddings to token values using cosine similarity
        # Get token embeddings: (vocab_size, d_model)
        token_embeddings = model.token_embedding.weight  # (vocab_size, d_model)

        # Normalize embeddings for cosine similarity
        best_output_norm = best_output / (best_output.norm(dim=-1, keepdim=True) + 1e-8)
        token_embeddings_norm = token_embeddings / (token_embeddings.norm(dim=-1, keepdim=True) + 1e-8)

        # Compute cosine similarity: (batch_size, 25, vocab_size)
        similarity = torch.matmul(best_output_norm, token_embeddings_norm.T)

        # Get token with highest similarity for each position
        decoded_tokens = similarity.argmax(dim=-1)  # (batch_size, 25)
...
def compute_loss_for_batch(
    ...
    """Compute loss for a single batch using denoising VAE.

    Looks up task and token embeddings, randomly corrupts 0-25 tokens in the
    output grid (last 25 tokens) with gaussian noise, and trains the model to
    reconstruct the clean embeddings. Task embedding and input grid are not corrupted.
    ...
    token_emb = model.token_embedding(batch)  # (batch_size, 50, d_model)

    # Concatenate: task token, input grid tokens, output grid tokens
    x_clean = torch.cat([task_emb, token_emb], dim=1)  # (batch_size, 51, d_model)

    # Randomly pick 0-25 tokens to corrupt in the output grid (last 25 tokens)
    ...
    # Generate gaussian noise for output tokens (batch_size, 25, d_model)
    noise = torch.randn(batch_size, 25, model.d_model, device=device)
    
    # Apply corruption to output grid (positions 26-50)
    x_corrupted[:, 26:, :] = torch.where(
        corruption_mask.unsqueeze(-1),
        noise,
        x_corrupted[:, 26:, :]
    )

    # Forward pass - model outputs denoised reconstruction and KL loss
    predicted, kl_loss = model(x_corrupted)  # (batch_size, 51, d_model), scalar
...
def train_epoch(
    ...
    Returns:
        Tuple of (average total loss, average reconstruction loss, average KL loss)
    """
    model.train()
    total_loss_sum = 0.0
    recon_loss_sum = 0.0
    ...
    for examples in train_loader:
        ...
        # Compute loss
        loss, recon_loss, kl_loss = compute_loss_for_batch(model, batch, task_indices, device)
    ...
    return total_loss_sum / num_batches, recon_loss_sum / num_batches, kl_loss_sum / num_batches
...
def train(config: Config):
    ...
    for epoch in range(start_epoch, start_epoch + config.num_epochs):
        ...
        # Train
        train_loss, train_recon_loss, train_kl_loss = train_epoch(model, train_loader, optimizer, device, task_id_to_index)

        # Test
        test_loss, test_recon_loss, test_kl_loss = test_epoch(model, test_loader, device, task_id_to_index)
        ...
        # Encoder transformer
        for layer_module in model.encoder_transformer.layers:
        ...
        # Decoder transformer
        for layer_module in model.decoder_transformer.layers:
            decoder_layer = cast(nn.TransformerEncoderLayer, layer_module)
            # Self-attention weights
            if decoder_layer.self_attn.in_proj_weight is not None:
                transformer_weights.append(decoder_layer.self_attn.in_proj_weight.flatten())
            transformer_weights.append(decoder_layer.self_attn.out_proj.weight.flatten())
            # Feedforward weights
            transformer_weights.append(decoder_layer.linear1.weight.flatten())
            transformer_weights.append(decoder_layer.linear2.weight.flatten())
        ...
        print(
            f"  Train - MSE: {train_recon_loss:.6f}, KL: {train_kl_loss:.6f} | "
            f"Test - MSE: {test_recon_loss:.6f}, KL: {test_kl_loss:.6f}"
        )
        print(
        ...
        writer.add_scalar("Loss/train_mse", train_recon_loss, epoch)
        writer.add_scalar("Loss/test_mse", test_recon_loss, epoch)
        writer.add_scalar("Loss/train_kl", train_kl_loss, epoch)
        writer.add_scalar("Loss/test_kl", test_kl_loss, epoch)
